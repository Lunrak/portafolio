import { EstadoCarta, DificultadMemoria, } from "./interfaces.js";
...
export function crearTablero(dificultad = DificultadMemoria.Facil) {
...
export function seleccionarCarta(tablero, cartaId, cartaSeleccionadaPrevia) {
    ...
    // Revelar la carta
    const cartasActualizadas = tablero.cartas.map((c) => c.id === cartaId ? { ...c, estado: EstadoCarta.Revelada } : c);
...
export function ocultarCartasReveladas(tablero) {
    return {
        ...tablero,
        cartas: tablero.cartas.map((c) => c.estado === EstadoCarta.Revelada
